...
<!-- <div class="ipsum abc" tabindex="0" width="42"> -->
```

<Callout type="warning">
Using `<Component {{ attrs.render() }}>` to pass the extra arguments to other components **WILL NOT WORK**. That is because the components are translated to function calls before the page render.
...
#### `.prepend_class(name1, name2, ...)`

Adds one or more classes to the beginning of the list of classes, if not already present.

```html+jinja
{% do attrs.prepend_class("hidden") %}
{% do attrs.prepend_class("active", "animated") %}
```
