...
- Both declarations are optional. If a component has more than one `{#css ... #}` or `{#js ... #}` declaration, the lists of files are merged.
...
The middleware uses the battle-tested [Whitenoise library](http://whitenoise.evans.io/) and will only respond to the *.css*, *.js*, and *.mjs* files inside the component(s) folder(s).
