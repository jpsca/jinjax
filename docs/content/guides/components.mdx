...
A "components/Person/Form.jinja" component is named "Person.Form", meaning the name of the subfolder and the name of the file separated by a dot. This is the full name you use to call it:
...
If a subfolder contains a component named `index.jinja`, you can call it using just the name of the subfolder:
...
The `Tab/index.jinja` component is named just "Tab", so you call it with `catalog.render("Tab")` or `<Tab> ... </Tab>`. The other components in the subfolder keep their regular names, e.g., `Tab/Panel.jinja` is still named "Tab.Panel".
