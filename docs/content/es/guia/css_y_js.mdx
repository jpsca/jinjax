...
{% Layout title="My page" %}
  {% Card %}
    ...
    {% endCardBody %}
    {% CardActions %}
      {% Button size="small" %}Share{% endButton %}
    {% endCardActions %}
  {% endCard %}
{% Layout %}
```
...
Puedes agregar etiquetas `<link>` y `<script>` en tu página automáticamente, llamando `catalog.render_assets()` en tu componente base, así:
...
Tus componentes podrían ser insertados dinámicamente en la página, después de que el JavaScript haya sido cargado y ejecutado. Por eso, conectar eventos a los elementos al cargar la página no será suficiente:
